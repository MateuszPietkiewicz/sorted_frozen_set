...
def test_index_one_before_the_beginning(sorted_frozen_set_sequence):
    with pytest.raises(IndexError):
...
def test_slice_from_end(sorted_frozen_set_sequence):
    assert sorted_frozen_set_sequence[3:] == SortedFrozenSet([13, 15])

def test_slice_from_empty(sorted_frozen_set_sequence):
...
def test_repetition_non_zero_left():
    s = SortedFrozenSet([1, 2, 3])
    assert s * 100 == SortedFrozenSet([1, 2, 3])
...
def test_repetition_non_zero_right():
    s = SortedFrozenSet([1, 2, 3])
    assert 100 * s == SortedFrozenSet([1, 2, 3])
